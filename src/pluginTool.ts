...
export function createListerTool(ctx?: OpenClawPluginToolContext) {
  ...
  return {
    ...
    promptGuidelines: [
      ...
      "For `itemCreate`, send the full item payload in `data` using the target list type's schema.",
      "For `itemUpdate`, send only the fields you want to change in `data`; each supplied field must match the target list type's schema.",
